...
with placeholder.container():
    ...
    if "load_state_7" and "Q7" and "Q7_ans" not in st.session_state:
        st.session_state["load_state_7"] = False
        st.session_state["Q7"] = []
        st.session_state["Q7_ans"] = []
    ...
    image, text, ask, select, answer = st.session_state['scam_operation'].return_values(question_no[6])
    ...
    if submit_answer or st.session_state.load_state_7:
        ...
        if answer_select == answer:
            ...
            if language == 'english':
                ...
                st.error(f"请向数码大使寻求帮助: 骗局问题 {question_no[6] + 1}")
        ...
        if submit_qns:
            st.session_state.Q7 = "scam" + " " + str(question_number)
            st.session_state.Q7_ans = correctness
            placeholder.empty()
